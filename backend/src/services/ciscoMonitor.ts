// Cisco device refresh: reads identity/health/ports/MAC-table/PoE/VLANs/
// neighbors over the pooled IOS/IOS-XE/NX-OS session and writes the shared
// tables (devices, device_metrics, ports, port_metrics, client_tracking,
// device_vlans, topology_links) so the rest of the app is vendor-agnostic.
// Mirrors routerosMonitor.ts; monitorService dispatches between them.
// vendor: cisco.
...
import { sshTargetFor, type DeviceRow } from './deviceComms.js';
...
import { evaluateHealthAlerts, decidePortFlap, shortName } from './monitorShared.js';

/** Full Cisco refresh. Mirrors refreshRouterOsDevice for IOS/IOS-XE/NX-OS gear.
 *  Takes the already-fetched device row (the dispatcher needed it to route). */
export async function refreshCiscoDevice(device: DeviceRow): Promise<void> {
