        ...
        {(['suggest', 'import'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`rounded-lg px-4 py-2 text-sm font-medium transition ${
              tab === t ? 'bg-brand-600 text-white dark:bg-brand-500' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-700/50'
            }`}>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-100 text-left dark:border-slate-800">
                      ...
                      <th className="pb-3 pr-4 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Platform</th>
                      <th className="pb-3 pr-4 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Protocol</th>
                      <th className="pb-3 pr-4 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Seen by</th>
                      {canAdd && <th className="pb-3"></th>}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                    {suggestions.map(s => (
                      <tr key={`${s.neighbor_ip}-${s.seen_by_ip}`} className="hover:bg-slate-50/80 transition dark:hover:bg-slate-800/60">
                        <td className="py-3 pr-4 font-medium text-slate-800 dark:text-slate-100">{s.neighbor_name || '—'}</td>
                        <td className="py-3 pr-4 font-mono text-xs text-slate-700 dark:text-slate-300">{s.neighbor_ip}</td>
                        <td className="max-w-64 truncate py-3 pr-4 text-slate-600 dark:text-slate-400" title={s.neighbor_platform}>{s.neighbor_platform || '—'}</td>
                        <td className="py-3 pr-4">
                          <span className={`rounded px-1.5 py-0.5 text-xs font-medium ${
                            s.protocol === 'cdp' ? 'bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400' : 'bg-purple-50 text-purple-700 dark:bg-purple-500/10 dark:text-purple-400'
                          }`}>{s.protocol.toUpperCase()}</span>
                        </td>
                        <td className="py-3 pr-4 text-xs text-slate-500 dark:text-slate-400">{s.seen_by_hostname || s.seen_by_ip}</td>
                        {canAdd && (
          ...
          <div className="space-y-4">
            <Card>
              <p className="mb-3 text-sm text-slate-600 dark:text-slate-400">
                Paste CSV with a header row. Required column: <code className="rounded bg-slate-100 px-1 py-0.5 text-xs dark:bg-slate-700/50">mgmt_ip</code>.
                Optional: <code className="rounded bg-slate-100 px-1 py-0.5 text-xs dark:bg-slate-700/50">hostname</code>,{' '}
                <code className="rounded bg-slate-100 px-1 py-0.5 text-xs dark:bg-slate-700/50">model</code>,{' '}
                <code className="rounded bg-slate-100 px-1 py-0.5 text-xs dark:bg-slate-700/50">credential_id</code>,{' '}
                <code className="rounded bg-slate-100 px-1 py-0.5 text-xs dark:bg-slate-700/50">site_id</code>.
              </p>
              <textarea
                ...
                <div className="space-y-1">
                  {importResults.map((r, i) => (
                    <div key={i} className={`flex items-center gap-2 rounded-lg px-3 py-2 text-sm ${
                      r.ok ? 'bg-green-50 text-green-800 dark:bg-green-500/10 dark:text-green-400' : 'bg-red-50 text-red-800 dark:bg-red-500/10 dark:text-red-400'
                    }`}>
