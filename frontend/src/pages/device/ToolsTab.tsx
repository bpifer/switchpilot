  ...
  return (
    <div className="space-y-4">
      <Card title="Network tools">
        {loading ? (
          <p className="text-sm text-slate-400 dark:text-slate-500">Loading tools…</p>
        ) : tools.length === 0 ? (
          <p className="text-sm text-slate-400 dark:text-slate-500">No diagnostic tools are available for this device.</p>
        ) : (
          <>
            <div className="flex flex-wrap items-end gap-3">
              <label className="block w-40">
                <span className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Tool</span>
              ...
              <label className="block min-w-56 flex-1">
                <span className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Target</span>
                <input
              ...
              {active !== 'ip-scan' && (
                <label className="block w-24">
                  <span className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Count</span>
                  <input
            ...
            <p className="mt-2 text-xs text-slate-400 dark:text-slate-500">{HINT[active]}</p>
            {!canOperate && (
              <p className="mt-1 text-xs text-amber-600 dark:text-amber-400">You need an operator role to run device tools.</p>
            )}
