      ...
      <Card title="Front panel">
        {ports.length > 0 && (
          <div className="mb-3 flex items-center gap-2">
            ...
            {q && <span className="text-xs text-slate-400 dark:text-slate-500">{shown.length} of {ports.length}</span>}
          </div>
        )}
        <div className="flex flex-wrap items-start gap-8">
          <PortGrid ports={shown} selected={selected} onSelect={setSelected} />
          {!port && ports.length > 0 && (
            <div className="self-center text-sm text-gray-400 dark:text-slate-500">
        ...
        <div className="space-y-3">
          <div>
            ...
            <div className="grid max-h-48 grid-cols-3 gap-1 overflow-auto sm:grid-cols-4 lg:grid-cols-6">
              {physical.map(p => (
                <label key={p.name}
                       className={`flex items-center gap-1.5 rounded border px-2 py-1 text-xs ${sel.includes(p.name) ? 'border-brand-300 bg-brand-50 text-brand-700 dark:border-brand-500/40 dark:bg-brand-500/10 dark:text-brand-400' : 'border-slate-200 text-slate-600 dark:border-slate-700 dark:text-slate-400'}`}>
          ...
          {progress && <p className="text-xs font-medium text-slate-500 dark:text-slate-400">{progress}</p>}
          {results.length > 0 && (
...
function LagPanel({ deviceId, ports, onChanged }: { deviceId: string; ports: Port[]; onChanged: () => void }) {
  ...
  return (
    <Card title="Link aggregation (LAG)">
      {lags.length > 0 && (
        <div className="mb-4">
          ...
          <div className="space-y-1">
            {lags.map(l => (
              <div key={l.name} className="flex items-center gap-3 rounded-lg border border-slate-200 px-3 py-1.5 text-sm dark:border-slate-700">
                <span className="font-mono font-medium text-slate-800 dark:text-slate-100">{l.name}</span>
                <StatusBadge status={l.oper_status} />
                {l.speed && <span className="text-xs text-slate-500 dark:text-slate-400">{l.speed}</span>}
                {l.description && <span className="truncate text-xs text-slate-400 dark:text-slate-500">{l.description}</span>}
                <button className={`${rowActionCls} ml-auto text-red-600 dark:text-red-400 disabled:opacity-50`}
      ...
      {deleting && (
        <div className="mb-4 rounded-lg border border-red-200 bg-red-50/50 p-3 dark:bg-red-500/10">
          <div className="mb-1 text-sm font-medium text-slate-800 dark:text-slate-100">Delete {deleting.name}</div>
          ...
          <div className="grid max-h-40 grid-cols-3 gap-1 overflow-auto sm:grid-cols-4 lg:grid-cols-6">
            {physical.map(p => (
              <label key={p.name}
                     className={`flex items-center gap-1.5 rounded border px-2 py-1 text-xs ${delMembers.includes(p.name) ? 'border-red-300 bg-red-50 text-red-700 dark:border-red-500/40 dark:bg-red-500/10 dark:text-red-400' : 'border-slate-200 text-slate-600 dark:border-slate-700 dark:text-slate-400'}`}>
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <div className="grid max-h-48 grid-cols-3 gap-1 overflow-auto sm:grid-cols-4 lg:grid-cols-6">
              {ports.map(p => (
                <label key={p.name}
                       className={`flex items-center gap-1.5 rounded border px-2 py-1 text-xs ${members.includes(p.name) ? 'border-brand-300 bg-brand-50 text-brand-700 dark:border-brand-500/40 dark:bg-brand-500/10 dark:text-brand-400' : 'border-slate-200 text-slate-600 dark:border-slate-700 dark:text-slate-400'}`}>
    ...
    <Card>
      ...
      <div className="flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-3">
          <span className="font-mono text-lg font-semibold text-slate-800 dark:text-slate-100">{port.name}</span>
          <StatusBadge status={port.oper_status} />
        </div>
        <span className="text-sm text-slate-500 dark:text-slate-400">
          {port.description || <span className="text-slate-300 dark:text-slate-600">no description</span>}
        </span>
        {canOperate && (
      ...
      <div className="mt-4">
        ...
        {macs.length > 0 ? (
          <div className="grid max-h-48 grid-cols-2 gap-x-4 gap-y-1 overflow-auto font-mono text-xs text-slate-600 sm:grid-cols-4 lg:grid-cols-6 dark:text-slate-400">
            {macs.map(m => <div key={m}>{m}</div>)}
          </div>
        ) : (
          <span className="text-sm text-slate-300 dark:text-slate-600">none</span>
        )}
      </div>
  ...
  return (
    <div className="mt-4">
      <div className="mb-2 flex items-center gap-3">
        <span className="text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">History</span>
        <div className="flex gap-1">
          {[24, 72, 168].map(h => (
            <button key={h}
              className={`rounded px-2 py-0.5 text-[11px] ${hours === h ? 'bg-brand-50 font-medium text-brand-700 ring-1 ring-brand-200 dark:bg-brand-500/10 dark:text-brand-400 dark:ring-brand-500/30' : 'text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-400'}`}
              onClick={() => setHours(h)}>
              {h === 24 ? '24h' : h === 72 ? '3d' : '7d'}
            </button>
          ))}
      ...
      {samples.length < 2 ? (
        <div className="rounded-lg border border-slate-100 bg-slate-50/60 py-6 text-center text-xs text-slate-400 dark:border-slate-800 dark:bg-slate-800/40 dark:text-slate-500">
        ...
        <div className="rounded-lg border border-slate-100 p-3 dark:border-slate-800">
          ...
          <div className="mt-2 flex h-2 w-full overflow-hidden rounded-sm" title="Link status over the period">
            {samples.map((s, i) => (
              <span key={i} className={`h-full flex-1 ${s.status === 'connected' ? 'bg-green-400' : s.status === 'err-disabled' ? 'bg-red-400' : 'bg-slate-200 dark:bg-slate-700'}`} />
            ))}
          </div>
...
function SfpOptics({ deviceId, portPath }: { deviceId: string; portPath: string }) {
  ...
  const header = (
    <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">Optics (SFP / DDM)</div>
  );
  if (data.present === false) {
    return <div className="mt-4">{header}<span className="text-sm text-slate-300 dark:text-slate-600">No transceiver inserted.</span></div>;
  }
  ...
  return (
    <div className="mt-4">
      ...
      {shown.length > 0 ? (
        ...
        <span className="text-sm text-slate-300 dark:text-slate-600">No optical data exposed by this port.</span>
      )}
    </div>
  );
}
...
export function PortConfigModal({ port, busy, onClose, onApply }: {
  ...
  return (
    <Modal title={`Configure ${port.name}`} onClose={onClose}>
      ...
      <div className="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">Switching</div>
      <div className="grid grid-cols-2 gap-3">
      ...
      <div className="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">Link</div>
      <div className="grid grid-cols-2 gap-3">
        <Field label="Speed">
      ...
      <div className="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">Spanning tree &amp; power</div>
...
function Stat({ label, value, sub, tone }: {
  ...
  return (
    <div className="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2 dark:border-slate-800 dark:bg-slate-800/40">
      ...
      <div className={`mt-0.5 text-sm font-medium ${tone === 'warn' ? 'text-amber-600 dark:text-amber-400' : 'text-slate-800 dark:text-slate-100'}`}>
        {value}
        {sub && <span className="ml-1.5 text-xs font-normal text-slate-400 dark:text-slate-500">{sub}</span>}
      </div>
    </div>
  );
}
